...
usamap <- p+theme(axis.text=element_blank(),
...
p <- ggplot() +
  geom_polygon(data=mydata, aes(x=long, y=lat, group=group, fill=(soyval12+1)), colour="black") +
...
```{r}
View(unique(cbind.data.frame(mydata$region[is.na(mydata$soyval12)],mydata$subregion[is.na(mydata$soyval12)])))
View(unique(counties$id))
View(unique(mydata$id[is.na(mydata$soyval12)]))
