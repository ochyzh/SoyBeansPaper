...
p<-ggplot(data=filter(trumpilot, is.finite(votech) & sales_USD>0), aes(x=sales_USD/1000000, y=exp(votech), label=mylabel))+geom_point()+
                geom_text_repel()+
							 geom_smooth(method=lm) +scale_y_continuous("Vote Change, logged") + scale_x_continuous("Soy Sales, millions USD")
p
...
#Acres,
trumpilot$mylabel<-trumpilot$state
trumpilot$mylabel[trumpilot$soyarea<2]<-""
...
#Sales, % GDP:
ggplot(data=trumpilot[is.finite(trumpilot$votech),], aes(x=log(soyval12v1+1), y=votech))+geom_point()+
							 geom_smooth(method=lm) +ylim(c(-2.5, 2.5))
