...
d2<-d2 %>% filter(`Data Item`=="CORN - ACRES PLANTED")%>% select(State, County, Value)%>% mutate(Value=str_remove(Value,","), county=tolower(County), state_name=paste0(toupper(substr(State, 1, 1)), tolower(substring(State, 2))), cornarea1617=as.numeric(Value)) %>% select(county, state_name, cornarea1617)
