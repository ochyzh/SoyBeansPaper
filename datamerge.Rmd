...
d<-d %>%
  ...
  summarise(FIPS=mean(FIPS),state_code=mean(state_code),state_name=first(state_name), county_code=first(county_code),Dem16=sum(Dem16),Rep16=sum(Rep16),Dem18=sum(Dem18),Rep18=sum(Rep18), Dist=min(Dist), Splitflag=as.numeric(length(Dist>1)), soyarea1617=0,soypct=0,soyval12=0, cornpct=NA,trumpch=NA, county_code=as.character(state_code)) %>% 
  ungroup
